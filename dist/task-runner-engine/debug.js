...
tr.app.UrlMatcher = function(a, b, c) {
  function d(b, c, d, e) {
    ...
    if (!/^\w+(-+\w+)*(?:\[\])?$/.test(b)) {
      throw Error("Invalid parameter name '" + b + "' in pattern '" + a + "'");
    }
    if (r[b]) {
      throw Error("Duplicate parameter name '" + b + "' in pattern '" + a + "'");
    }
    r[b] = new $$UMFP.Param(b, c, d, e);
    return r[b];
  }
  function e(a, b, c, d) {
    var e = ["", ""];
  ...
  for (var q;m = g.exec(a);) {
    m = f(m, !1);
    if (0 <= m.segment.indexOf("?")) {
      break;
    }
    q = d(m.id, m.type, m.cfg, "path");
    k += e(m.segment, q.type.pattern.source, q.squash, q.isOptional);
    p.push(m.segment);
    l = g.lastIndex;
  }
  c = a.substring(l);
  m = c.indexOf("?");
  if (0 <= m) {
    var v = this.sourceSearch = c.substring(m);
    c = c.substring(0, m);
    this.sourcePath = a.substring(0, l + m);
    if (0 < v.length) {
      for (l = 0;m = h.exec(v);) {
        m = f(m, !0), q = d(m.id, m.type, m.cfg, "search"), l = g.lastIndex;
      }
    }
  } else {
  ...
  k += e(c) + (!1 === b.strict ? "/?" : "") + "$";
  p.push(c);
  this.regexp = new RegExp(k, b.caseInsensitive ? "i" : void 0);
  this.prefix = p[0];
  this.$$paramNames = t;
};
...
tr.app.UrlMatcher.prototype.exec = function(a, b) {
  ...
  b = b || {};
  var e = this.parameters(), f = e.length, g = this.segments.length - 1, h = {}, k, l, m;
  if (g !== d.length - 1) {
    throw Error("Unbalanced capture group in route '" + this.source + "'");
  }
  for (k = 0;k < g;k++) {
    m = e[k];
    var p = this.params[m], n = d[k + 1];
    for (l = 0;l < p.replace;l++) {
      p.replace[l].from === n && (n = p.replace[l].to);
    }
    n && !0 === p.array && (n = c(n));
    h[m] = p.value(n);
  }
  for (;k < f;k++) {
    m = e[k], h[m] = this.params[m].value(b[m]);
  }
  return h;
};
...
tr.app.UrlMatcher.Type_.prototype.$asArray = function(a, b) {
  ...
  if ("auto" === a && !b) {
    throw Error("'auto' array mode is for query parameters only");
  }
  return new function(a, b) {
    ...
    this.encode = h(e(a, "encode"));
    this.decode = h(e(a, "decode"));
    this.is = h(e(a, "is"), !0);
    ...
    this.$normalize = h(e(a, "$normalize"));
    this.name = a.name;
    this.$arrayMode = b;
  }(this, a);
};
tr.app.UrlMatcher.UrlMatcherFactory_ = function() {
  ...
  var e = !1, f = !0, g = !1, h = {}, k = !0, l = [], m, p = {string:{encode:a, decode:function(a) {
    ...
    return this.is(a) ? a : (a = this.capture.exec(a)) ? new Date(a[1], a[2] - 1, a[3]) : void 0;
  }, is:function(a) {
  ...
  this.Param = function(a, c, d, e) {
    function f() {
      ...
      if (null !== a && void 0 !== a && !k.type.is(a)) {
        throw Error("Default value (" + a + ") for parameter '" + k.id + "' is not an instance of Type (" + k.type.name + ")");
      }
      return a;
    }
    var k = this;
    ...
    var p = void 0 !== d.value, u = function(a, b) {
      var c = a.squash;
      if (!b || !1 === c) {
        return!1;
      }
  ...
  d.prototype = {$$new:function() {
    ...
    var b = this.$$keys(), c, d, e;
    for (c = 0;c < b.length;c++) {
      d = this[b[c]];
      e = a[b[c]];
      if ((void 0 === e || null === e) && d.isOptional) {
        break;
      }
      e = d.type.$normalize(e);
      if (!d.type.is(e)) {
        return!1;
      }
      e = d.type.encode(e);
...
tr.enums.State = {INITIALIZED:"INITIALIZED", RUNNING:"RUNNING", INTERRUPTED:"INTERRUPTED", COMPLETED:"COMPLETED", ERRORED:"ERRORED"};
...
tr.Decorator = function(a, b) {
  tr.Abstract.call(this, b || "Decorator");
  goog.asserts.assert(goog.isFunction(a.run), 'Required method "run" not implemented.');
  this.decorated_ = a;
};
...
tr.Decorator.prototype.runImpl = function() {
  this.decorated_.run(goog.bind(this.complete_, this), goog.bind(this.error_, this));
};
tr.Decorator.prototype.interruptImpl = function() {
  goog.isFunction(this.decorated_.interrupt) && this.decorated_.interrupt();
};
tr.Decorator.prototype.resetImpl = function() {
  goog.isFunction(this.decorated_.reset) && this.decorated_.reset();
};
tr.Decorator.prototype.complete_ = function(a) {
  this.completeInternal(a);
};
tr.Decorator.prototype.error_ = function(a, b) {
  this.errorInternal(a, b);
};
...
tr.Graph.prototype.addBlockersTo = function(a, b) {
  this.updateBlockers_(b, a);
  return this;
};
tr.Graph.prototype.remove = function(a) {
  this.verifyInGraph_([a]);
  this.removeCallbacksFrom_(a);
  this.tasks_.splice(this.tasks_.indexOf(a), 1);
  delete this.taskIdToDependenciesMap_[a.getUniqueID()];
  for (var b in this.tasks_) {
    this.validateDependencies_(this.tasks_[b]);
  }
  this.getState() == tr.enums.State.RUNNING && this.completeOrRunNext_();
  return this;
};
tr.Graph.prototype.removeAll = function(a) {
  for (var b = 0, c = a.length;b < c;b++) {
    this.remove(a[b]);
  }
  return this;
};
tr.Graph.prototype.removeBlockersFrom = function(a, b) {
  this.verifyInGraph_(a);
  this.verifyInGraph_(b);
  for (var c = this.taskIdToDependenciesMap_, d = 0, e = b.length;d < e;d++) {
    for (var f = b[d], g = c[f.getUniqueID()] || [], d = 0, e = a.length;d < e;d++) {
      var h = g.indexOf(a[d]);
      0 <= h && g.splice(h, 1);
    }
    c[f.getUniqueID()] = g;
...
tr.Graph.prototype.updateBlockers_ = function(a, b) {
  if (b && 0 !== b.length) {
    this.verifyInGraph_(a);
    this.verifyInGraph_(b);
    for (var c = 0, d = a.length;c < d;c++) {
      var e = a[c];
      goog.asserts.assert(e.getState() !== tr.enums.State.RUNNING, "Cannot add blocking dependency to running task.");
      for (var f = this.taskIdToDependenciesMap_[e.getUniqueID()] || [], c = 0, d = b.length;c < d;c++) {
        var g = b[c];
        0 > f.indexOf(g) && f.push(g);
      }
      this.taskIdToDependenciesMap_[e.getUniqueID()] = f;
    }
  }
};
tr.Graph.prototype.validateDependencies_ = function(a) {
  var b = this.taskIdToDependenciesMap_[a.getUniqueID()];
  b && (goog.asserts.assert(0 > b.indexOf(a), "Cyclic dependency detected."), this.verifyInGraph_(b));
};
tr.Graph.prototype.verifyInGraph_ = function(a) {
  for (var b = 0, c = a.length;b < c;b++) {
    goog.asserts.assert(0 <= this.tasks_.indexOf(a[b]), "Task not in graph.");
  }
};
...
tr.Promise = function(a, b) {
  goog.asserts.assert(goog.isDef(a), "Invalid promise provided");
  tr.Abstract.call(this, b || "Promise");
  this.promise_ = a;
  if (tr.Promise.isAngularDetected_()) {
    this.observeForAngular_(a);
  } else {
    if (tr.Promise.isES6Detected_()) {
      this.observeForES6_(a);
    } else {
      if (tr.Promise.isJQueryDetected_()) {
        this.observeForJQuery_(a);
      } else {
        if (tr.Promise.isQDetected_()) {
          this.observeForQ_(a);
        } else {
...
tr.Promise.promiseToTask = function(a, b) {
  return new tr.Promise(a, b);
};
tr.Promise.taskToPromise = function(a, b) {
  goog.asserts.assert(goog.isDef(a), "Invalid task provided");
  if (tr.Promise.isAngularDetected_()) {
    return this.createAngularPromise_(a, b);
  }
  if (tr.Promise.isES6Detected_()) {
    return this.createES6PromisePromise_(a);
  }
  if (tr.Promise.isJQueryDetected_()) {
    return this.createJQueryPromise_(a);
  }
  if (tr.Promise.isQDetected_()) {
    return this.createQPromise_(a);
  }
  throw "No supported Promise libraries detected.";
};
tr.Promise.prototype.runImpl = goog.nullFunction;
tr.Promise.prototype.completeIfRunning_ = function(a) {
  if (this.getState() === tr.enums.State.RUNNING) {
    this.completeInternal(a);
  } else {
    var b = goog.bind(function() {
      this.completeInternal(a);
      this.off(b, "started");
    }, this);
    this.started(b);
...
tr.Promise.prototype.errorIfRunning_ = function(a, b) {
  if (this.getState() === tr.enums.State.RUNNING) {
    this.errorInternal(a, b);
  } else {
...
tr.Promise.prototype.observeForAngular_ = function(a) {
  var b = this;
  a.then(function(a) {
    b.completeIfRunning_(a);
  }, function(a) {
    b.errorIfRunning_(a, a);
  });
};
tr.Promise.prototype.observeForES6_ = function(a) {
  var b = this;
  a.then(function(a) {
    b.completeIfRunning_(a);
  }, function(a) {
...
tr.Promise.prototype.observeForJQuery_ = function(a) {
  var b = this;
  a.then(function(a) {
    b.completeIfRunning_(a);
  }, function(a) {
...
tr.Promise.prototype.observeForQ_ = function(a) {
  var b = this;
  a.then(function(a) {
    b.completeIfRunning_(a);
  }, function(a) {
    b.errorIfRunning_(a, a.message || a);
  });
};
tr.Promise.createAngularPromise_ = function(a, b) {
  goog.asserts.assert(goog.isDef(b), "Invalid $q provided");
  var c = b.defer();
  a.completed(function(a) {
    c.resolve(a.getData());
  });
  a.errored(function(a) {
    c.reject(a.getErrorMessage());
  });
  return c.promise;
};
tr.Promise.createES6PromisePromise_ = function(a) {
...
tr.Promise.createJQueryPromise_ = function(a) {
  var b = new jQuery.Deferred;
  a.completed(function(a) {
    b.resolve(a.getData());
  });
  a.errored(function(a) {
    b.reject(a.getErrorMessage());
  });
  return b.promise();
};
tr.Promise.createQPromise_ = function(a) {
  var b = Q.defer();
  a.completed(function(a) {
    b.resolve(a.getData());
  });
...
tr.StopOnSuccess = function(a, b) {
  tr.Abstract.call(this, b || "StopOnSuccess");
  this.taskQueue_ = [];
  this.taskQueueIndex_ = 0;
  this.completedTasks_ = [];
  this.erroredTasks_ = [];
...
tr.StopOnSuccess.prototype.addAll = function(a) {
  for (var b = 0;b < a.length;b++) {
    this.add(a[b]);
  }
  return this;
};
tr.StopOnSuccess.prototype.add = function(a) {
  goog.asserts.assert(this.getState() !== tr.enums.State.RUNNING, "Cannot add task while running.");
  if (0 <= this.taskQueue_.indexOf(a)) {
    throw "Cannot add task more than once.";
  }
  this.taskQueue_.push(a);
  return this;
};
tr.StopOnSuccess.prototype.remove = function(a) {
...
tr.StopOnSuccess.prototype.addCallbacks_ = function(a) {
  a.completed(this.childTaskCompleted_, this);
  a.errored(this.childTaskErrored_, this);
};
tr.StopOnSuccess.prototype.removeCallbacks_ = function(a) {
  a.off(tr.enums.Event.COMPLETED, this.childTaskCompleted_, this);
  a.off(tr.enums.Event.ERRORED, this.childTaskErrored_, this);
};
...
tr.StopOnSuccess.prototype.checkForTaskCompletion_ = function() {
  0 < this.completedTasks_.length ? this.completeInternal() : this.completedTasks_.length + this.erroredTasks_.length >= this.taskQueue_.length && (0 < this.erroredTasks_.length ? this.errorInternal() : this.completeInternal());
};
tr.StopOnSuccess.prototype.taskCompletedOrRemoved_ = function(a) {
  this.taskQueueIndex_++;
  this.getState() == tr.enums.State.RUNNING && (this.checkForTaskCompletion_(), this.getState() == tr.enums.State.RUNNING && (a = this.taskQueue_[this.taskQueueIndex_])) && (this.addCallbacks_(a), a.run());
};
tr.StopOnSuccess.prototype.eachTaskInQueue_ = function(a) {
  for (var b = 0;b < this.taskQueue_.length;b++) {
    a(this.taskQueue_[b]);
  }
};
tr.StopOnSuccess.prototype.childTaskCompleted_ = function(a) {
  this.completedTasks_.push(a);
  this.taskCompletedOrRemoved_(a);
};
tr.StopOnSuccess.prototype.childTaskErrored_ = function(a) {
  this.erroredTasks_.push(a);
  this.taskCompletedOrRemoved_(a);
};
tr.Chain = function(a, b, c) {
  tr.Abstract.call(this, c || "Chain");
  ...
  void 0 !== a && this.completed(a);
  void 0 !== b && this.errored(b);
};
goog.inherits(tr.Chain, tr.Abstract);
tr.Chain.prototype.getDecoratedTask = function() {
  return this.graph_;
};
tr.Chain.prototype.first = function(a) {
  goog.asserts.assert(0 === this.graph_.getOperationsCount(), "Cannot call first after tasks have been added");
...
tr.Chain.prototype.else = function(a) {
  return this.or.apply(this, arguments);
};
tr.Chain.prototype.otherwise = function(a) {
  return this.or.apply(this, arguments);
};
...
tr.app.TransitionState.prototype.beforeFirstRun = function() {
  this.addToEnd(new tr.Closure(goog.bind(this.chooseState_, this), !1, "Closure - state-chooser"));
};
tr.app.TransitionState.prototype.addTargetState = function(a, b) {
  b = b || [];
  this.prioritizedStates_.push(a);
  this.taskIdToBlockingTasksMap_[a.getUniqueID()] = b;
  for (var c = 0;c < b.length;c++) {
    var d = b[c];
    0 <= this.blockingTasks_.indexOf(d) || (this.blockingTasks_.push(d), this.add(new tr.Failsafe(d)));
  }
  return this;
};
